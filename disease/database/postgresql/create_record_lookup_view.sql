...
) das ON gc.concept_id::text = das.concept_id::text
FULL JOIN disease_labels dl ON gc.concept_id::text = dl.concept_id::text
FULL JOIN (
    SELECT dx_1.concept_id, array_agg(dx_1.xref) AS xrefs
    FROM disease_xrefs dx_1
    GROUP BY dx_1.concept_id
) dx ON gc.concept_id::text = dx.concept_id::text;
